...
describe('API /usuarios :: CRIAÇÃO (NEGATIVOS → POSITIVOS)', () => {
  ...
  before(() => {
    cy.fixture('usuarios.create.cases.json').then((data) => (fx = data));
    cy.fixture('schema/usuario.create.success.schema.json').then((s) => (schemaSuccess = s));
    cy.fixture('schema/usuario.create.emailInUse.schema.json').then((s) => (schemaEmailInUse = s));
  });
